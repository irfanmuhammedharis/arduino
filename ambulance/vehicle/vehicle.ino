...
// Vibration sensor pin and threshold
#define VIBRATION_SENSOR_PIN 34 // ADC1 channel pin
#define VIBRATION_THRESHOLD 500 // Adjust based on sensor's range
...
void loop() {
  ...
  // Check if vibration crosses threshold
  if (vibrationValue > VIBRATION_THRESHOLD) {
    // Check if GPS location is valid
    if (gps.location.isUpdated()) {
      myData.id = 3; // Unique ID for the sender
    ...
    Serial.println("Vibration below threshold, no data sent.");
  }

  delay(200); // Small delay to avoid excessive sensor polling
}
